...
// customer's submitted code against, and what RequestSMSFallback updates
// in place when falling back from Telegram. Excludes already-verified
// requests (a stale success shouldn't be re-checked) and expired ones (an
// old request from an earlier attempt shouldn't linger as valid just
// because nothing deleted it yet).
